...
var Slider = Class.extend({
  init: function($slider_view){
    // The two divs that will contain photos, one is hidden;
    this.imageDiv = $slider_view;
    this.delaySecs = $('[data-slider_delaysecs]').data("slider_delaysecs");
    this.hiddenCatalog = $('[data-slider_catalog]');
    // All the slider images
    this.sliderImages = $('[data-slider_image]');
    this.imageLength = this.sliderImages.length;
    this.getSwapMethods();
    // Init to add current status to first image.
    this.getNavType();

    // Updates max num photos text
    $('[data-slider_maxnum]').text(this.imageLength);
    // If no image is shown. Display the first.
    if($slider_view.is(':empty')){

      if($('[data-slider_current="true"]').length == 0) {
        $selected_image = this.sliderImages.first();
      } else {
        $selected_image = $('[data-slider_current="true"]');
      }
      this.setCurrent($selected_image);
      this_nav = new this.nav_type($('[data-slider_nav]'), this);
      this_nav.checkBounds($(this.findCurrent()).data('slider_image'), this);
    }
  },
  getNavType: function(){
    this.nav_type = SliderNav;
  },
  getSwapMethods: function(){
    this.in_method = this.imageDiv.data('in_method');
    if(!this.in_method){
      this.in_method = 'fadeIn';
    };
    this.out_method = this.imageDiv.data('out_method');
    if(!this.out_method){
      this.out_method = 'fadeOut';
    };
  },
  changeCurrent: function(sliderDirection) {
    // If there is no current image. Set the first image to current.
    if(this.findCurrent() == null) {
      return true;
    } else {
      // Find the old and new current.
      var oldCurrent = $(this.findCurrent());
      var oldCurrentNum = oldCurrent.data('slider_image');
      var newCurrent = $(this.sliderImages[oldCurrentNum + sliderDirection]);
      var hiddenCatalog = this.hiddenCatalog;
      // Change the old and new status.
      oldCurrent.data('slider_current', false);
      oldCurrent.css({'position':'absolute','top':'0','left':'0'});
      // Change old status and remove it from the visible div.
      oldCurrent[this.out_method]("slow", function() {
        // Depending on the navigation direction. It will append differently to the hidden div.
        if (sliderDirection == 1) {
          $(hiddenCatalog).append(oldCurrent);
        } else {
          $(hiddenCatalog).prepend(oldCurrent);
        }
      });
  ...
  setCurrent: function(newCurrent){
    var newCurrentNum = newCurrent.data('slider_image');
    var newSubtitle = newCurrent.data('subtitle');
    if(!newSubtitle){
      newSubtitle = ''
    }
    // Change the status of the next image to current and add it to the visible div.
    newCurrent.data('slider_current', true);
    this.imageDiv.prepend(newCurrent);
    newCurrent.css({'position':'relative'});
    newCurrent[this.in_method]("slow", function() {
    });
    $('[data-slider_subtitle]').text(newSubtitle);
    $('[data-slider_currentnum]').text(newCurrentNum + 1);
  },
  findCurrent: function() {
    // Finds the image with a true current status.
    var currentItem;
    this.sliderImages.each(function() {
      if($(this).data('slider_current') == true) {
        currentItem = this;
        return false;
      };
    });
...
var SliderNav = Class.extend({
  init: function($slider_nav, slider){
    // Decides wheather to automaticly rotate images
    if($("[data-slider_autochange]").data("slider_autochange")) {
     this.userTouch = false;
     this.changeAutomatic(slider);
    }
    this.sliderNav = $slider_nav;
    var functionThis = this;
    // Binds click to nav bars and will change current image when clicked.
    $slider_nav.click(function() {
      functionThis.slide(this,slider);
    });
  },
  slide: function(clicked_nav,slider){
    if(clicked_nav != "auto") { clearInterval(this.autoRotateInterval) }
    var imageNum = $(slider.findCurrent()).data('slider_image');
    // Determines the direction of slider navigation.
    if(clicked_nav == "auto") {
      var sliderDirection = (imageNum == slider.imageLength - 1)  ? -slider.imageLength + 1 : 1;
    } else {
      var sliderDirection = $(clicked_nav).data('slider_nav') == "left" ? -1 : 1;
    }

    // Changes the current photo to the next based on direction of click
    slider.changeCurrent(sliderDirection);
    // Checks the bounds and determines if nav bars should be shown.
    var newCurrent = slider.findCurrent();
    this.checkBounds($(newCurrent).data("slider_image"), slider);

  },
  checkBounds: function(imageNum, slider) {
    // Hides individual nav bars if slider is at the begginning or end
    // of the images to display.
    if(imageNum == 0) {
      $(this.sliderNav).filter('[data-slider_nav="right"]').removeClass('hidden');
      $(this.sliderNav).filter('[data-slider_nav="left"]').addClass('hidden');
    } else if (imageNum == slider.imageLength - 1) {
      $(this.sliderNav).filter('[data-slider_nav="right"]').addClass('hidden');
    } else {
      $(this.sliderNav).removeClass('hidden');
    }
  },
  changeAutomatic: function(slider) {
    var globalThis = this;
    this.autoRotateInterval = setInterval(function() {
      globalThis.slide("auto", slider);
    }, slider.delaySecs * 1000);
  }
});

var LocationSlider = Slider.extend({
  getNavType: function(){
    this.nav_type = LocationSliderNav;
  }
});

var LocationSliderNav = SliderNav.extend({
  slide: function(clicked_nav,slider){
    this._super(clicked_nav,slider);
    var $target_search_location = $('[data-index="' + $(slider.findCurrent()).data('slider_image') + '"]')
    var $this_logo = $($target_search_location.data('logo')).clone();
    $this_logo.hide();
    $('#logo_container').html($this_logo);
    $this_logo.fadeIn('slow');
    $('#current_search_location').html($target_search_location.html() + '<div class="triangle"></div>');
  }
});

$(document).ready(function() {
  $('[data-slider_view]').each(function() {
    var this_class = eval(getSubClass($(this).data('sub-type'),'Slider'));
    new this_class($(this));
  });
});
...
var UploadTrigger = Class.extend({
  init: function(jq_obj){
    var form = jq_obj.parents('form');
    var ih_server_endpoint = form.data("ih-server-endpoint");
    var form_data_array = form.serializeArray();
    var form_data = arrayToObject(form_data_array);
    var target = jq_obj.data('target');
    var queue = jq_obj.data('queue');
    var location_type = form.data('location-type');
    var location_id = form.data('location-id');
    var file_info = {};
    jq_obj.uploadifive({
      'auto'             : true,
      ...
      'onUploadFile'     : function(file) {
        var file_slug = convertToSlug(file.name);
        var new_uuid = new UUID;
        var uniq_id = new_uuid.value;
// TODO: find out what other attributes of the file might be uniq to avoid problems if someone uploads 2 images with the same filename.
        file_info[file_slug] = uniq_id;
        var this_form_data = $.extend({},form_data);
        this_form_data.key = form_data.key + uniq_id + '/' + file_slug;
        jq_obj.uploadifive("settings", {'formData': this_form_data});
      },
      'onUploadComplete' : function(file, data) {
        var file_slug = convertToSlug(file.name);
        var ih_form_data = {'location_photo[location_type]': location_type, 'location_photo[location_id]': location_id};
        ih_form_data.new_file_uniq_id = file_info[file_slug];
        ih_form_data.new_file_name = file_slug;
        $.post(
          ih_server_endpoint,
          ih_form_data,
...
var UploadCsvTrigger = Class.extend({
  init: function(jq_obj){
    var form = jq_obj.parents('form');
    var target = "#" + jq_obj.data('target');
    var queue = jq_obj.data('queue');
    ...
    jq_obj.uploadifive({
      'auto'             : true,
      'removeCompleted'  : true,
      'fileObjName'      : jq_obj.data('file_field'),
      'formData'         : {authenticity_token: $('[name="csrf-token"]').attr('content')},
      'queueID'          : queue,
      'uploadScript'     : form.attr('action'),
      'method'           : 'put',
      'onUploadComplete' : function(file, data) {
        $("#uploadifive-manage_import_" + jq_obj.data('target')).hide();
        $(target).append("Uploaded");
      },
      'simUploadLimit'   : 1,
      'buttonClass'      : "btn btn-primary",
...
var Expandable = Class.extend({
  init: function($expandable){
    ...
    $collapser = $('[data-expand_id="' + $expandable.data('expand_id') + '"][data-collapser]');
    if($collapser.length > 0){
      this.collapser = new Collapser($collapser,this);
    }
    this.$expander = $('[data-expand_id="' + $expandable.data('expand_id') + '"][data-expander]');
...
var TabTrigger = Class.extend({
  ...
  toggleTarget: function(){
    ...
    this.tab_group.hideAll();
    if(!was_visible){
      this.tab_group.deactivateTabTriggers();
      this.$tab_trigger.addClass('active');
      this.target.show();
    }

    if($('.filter').width() < 480) {
      this.target.css("left", this.target.data('tab-id') == "bedrooms_filter" ? -124 : 0)
    }
  },
  hideTarget: function(){
    this.target.hide();
...
var AjaxTabTrigger = TabTrigger.extend({
  toggleTarget: function(){
    if(!this.ajax_loaded){
      ...
      new AjaxLinkSubmission(this.$tab_trigger);
    }
    this._super();
  }
})
...
var StartDateSetter = Class.extend({
  init: function($start_date_setter){
    this.$start_date_setter = $start_date_setter;
    this.$end_date = $($start_date_setter.data('end_date'));
    var specific_min_period = $start_date_setter.data('minimum_period');
    if(specific_min_period){
      this.minimum_period = parseInt(specific_min_period);
    } else {
      this.minimum_period = 1;
    }
    var start_date_setter = this;
    $start_date_setter.bind('change',function(){
      start_date_setter.setEndDate();
    })
  },
  setEndDate: function(){
    var date_parts = this.$start_date_setter.val().split('/');
    var start_day = parseInt(date_parts[1]);
    var default_end_day = start_day + this.minimum_period;
    date_parts[1] = default_end_day.toString();
    var default_end_date = date_parts.join('/');
    this.$end_date.val(default_end_date);
  }
});
...
$(document).ready(function(){
  ...
  $('[data-tab-group]').each(function(){
    var tab_group = this;
    var this_class = eval(getSubClass($(tab_group).data('sub-type'),'TabGroup'));
    new this_class($(tab_group));
  })
...
$(document).ready( function() {
  ...
  var availableDates = $('[data-available_days]').data();

  $(filterToggle).click(function() {
    $('.filter').toggle();
  });

  // Hides all filter selection if click is outside of an box;
  $(document).click(function(e) {
    if(!$.browser.msie && !$.browser.firefox && !$(e.toElement).parents('.filter-box').length) {
     $('.filter-adjbox').hide();
  ...
  $(window).bind("scroll", function(e) {
    ...
    changeFilterBarPosition(null);
  });

  submitButtonVisibility();
  $(window).bind("resize", submitButtonVisibility)

});

var submitButtonVisibility = function() {
  if($(".filter").width() < 480) {
...
var changeDayText = function(day, date, available_dates)
{
  $(".cal_date_available").each(function() {
    if($(this).hasClass("day_" + day)) {
      var rate_and_min_stay = searchMultiArray(date, available_dates);
      if (rate_and_min_stay != undefined) {
        $(this).addClass("date_with_rate");
        $(this).find("span").remove();
        $(this).append("<span data-date_for_rate=" + date + " data-min_stay=" + rate_and_min_stay[1] + ">$" + rate_and_min_stay[0] + "</span>");
      }
    }
  });
}

var searchMultiArray = function(date, available_dates) {
  for(var i = 0; i < available_dates.length; i++) {
    if(available_dates[i].day === date) {
      return [available_dates[i].rate, available_dates[i].min_stay];
    }
  }
}

var toggleArrowDirection = function (parent) {
  var displayedDiv = parent.find('.filter-adjbox');
  var allArrows = $('.filter-arrow');
  var arrow = parent.find('.filter-arrow');

  // If the div is displayed, flip the selected sections arrow and return all others
  // to their original position.
  allArrows.css({'border-bottom': '0', 'border-top': '5px solid #000'});

  if(displayedDiv.is(":visible")) {
    arrow.css({'border-top': '0', 'border-bottom': '5px solid #000'});
  }
};

// Changes the filter bar position between relative and fixed based on scroll position.
var changeFilterBarPosition = function (calender) {
  var filterBar = $(".filter");
  // Won't change position for the iPhone
  if(filterBar.width() > 480) {
    var distanceTop = $(window).scrollTop();
    var searchDiv = $('.search');
    if(distanceTop >= 115) {
      filterBar.css({"position": "fixed", "margin-top": '0'});
      $("#ui-datepicker-div").css("position", "fixed").css("top", "24px");
      changeFilterBarWidth();
    } else {
      filterBar.css({"position": "relative", "margin-top": '-33px'});
      changeFilterBarWidth();
      $("#ui-datepicker-div").css("position", "absolute").css("top", "128px");
    }
  } else {
    if(calender) {
      $("#ui-datepicker-div").css("left", $(calender).offset().left + 80);
    }
  }
};

// Resizes filter bar to line up with Unit width.
var changeFilterBarWidth = function () {
  var filterBar = $(".filter");
  filterBar.width('960px');
};

// Loads and populates calender
var loadAndFillCal = function(calender) {
  var input = calender.find('input');
  var selectedDate = input.datepicker("getDate");
  var arrivalDate = $('.filter-arrival input').datepicker("getDate");
  var today = new Date();
  var minDate = (arrivalDate < today) ? today : arrivalDate
  var reservedDates = $('[data-reserved_days]').data();
  var availableDates = $('[data-available_days]').data();

  // Hide all other filter boxes
  var allSliders = $('.filter-adjbox');
  allSliders.hide();
  ...
  input.datepicker({
    ...
    beforeShowDay: function(date) {
      ...
      setTimeout(function(){
        if(availableDates) {
          changeDayText(day, date, availableDates.available_days);
        }
      }, 100);
      if (reservedDates != null) {
        var date = jQuery.datepicker.formatDate('yy-mm-dd', date);
        if (reservedDates.reserved_days.indexOf(date) == -1) {
    ...
    onSelect: function(date){
      if(calender.selector == '.filter-arrival') {
        dateRearrange = date.split('/')
        dateRearrange.unshift(dateRearrange.pop());
      }
      populateDateText(calender, input);
      if($(".filter").width() > 480) {
        input.trigger('change');
      }
    },
    onClose: function(){
      if(calender.selector == '.filter-arrival' & input.datepicker("getDate") != null) {
  ...
  if(calender.selector == '.filter-departure') {
...
// Updates date values in filter bar
var populateDateText = function(parent, input) {
  var date = input.datepicker("getDate");
  if(date != null) {
    var dayText = ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][date.getDay()];
    var dayNum = date.getDate();
    var month = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][date.getMonth()];
    var year = date.getFullYear();

    parent.find('.filter-selection').text(dayText + "., " + month + " " + dayNum + ", " + year);
  }
};

var Mirror = Class.extend({
  init: function($mirror_source){
    this.$mirror_source = $mirror_source;
    this.$mirror = $($mirror_source.data('mirror'));
    $mirror_source.data('mirror',this);
    var mirror_source = this;
    $mirror_source.change(function(){
      mirror_source.reflect();
    })
  },
  reflect: function(){
    switch(this.$mirror.get(0).nodeName.toLowerCase()){
      case 'input': case 'select':
        this.$mirror.val(this.$mirror_source.val());
        break;
      default:
        this.$mirror.text(this.$mirror_source.val());
        break;
    }
  }
});

var SliderInput = Class.extend({
  init: function($slider){
    this.$slider = $slider;
    this.low_end = $slider.data('low-end');
    this.high_end = $slider.data('high-end');
    this.low_start = $slider.data('low-start');
    this.high_start = $slider.data('high-start');
    this.low_end_input = $($slider.data('low-end-input'));
    this.high_end_input = $($slider.data('high-end-input'));
    this.step = $slider.data('step');
    var slider = this;
    $slider.noUiSlider({
      range: [slider.low_end,slider.high_end],
      start: [slider.low_start,slider.high_start],
      handles: 2,
      step: slider.step,
      serialization: {
        to: [slider.low_end_input,slider.high_end_input],
        resolution: 1
      },
      slide: function(){
        slider.low_end_input.data('mirror').reflect();
        slider.high_end_input.data('mirror').reflect();
      }
    });

    if($(".filter").width() > 480) {
      $slider.bind('mouseup',function(){
        slider.low_end_input.trigger("change");
        return false;
      });
      $slider.parents('[data-tab-id]').bind('mouseleave',function(){
        slider.low_end_input.trigger("change");
        return false;
      });
    }
...
var CollaborativeFieldFiller = FieldFiller.extend({
  fill: function(e){
    this.target.val(this.value);
    if (this.target.selector == "input[name=search_location_id]") {
      $(".filter-location .filter-selection").text(this.$field_filler[0].innerText);
    }
    if($(".filter").width() > 480) {
...
var RangeSubmitter = Class.extend({
  init: function($submitter){
    this.start = new RangePart($($submitter.data('start')),this);
    this.end = new RangePart($($submitter.data('end')),this);
    this.target = $submitter.parents('form:first');
  },
  trySubmit: function(){
    if(this.start.isSet() && this.end.isSet()){
      this.target.submit();
    }
  }
});

var BookDateRangeSubmitter = RangeSubmitter.extend({
  trySubmit: function(){
    if(this.start.isSet() && this.end.isSet()){
      $('[data-receipt-main]').html($('[data-property-loader]').show());
      this.target.submit();
    }
  }
});
var RangePart = Class.extend({
  init: function($input,submitter){
    this.$input = $input;
    this.submitter = submitter;
    var input = this;
    $input.on('change',function(){
      input.submitter.trySubmit();
    })
  },
  isSet: function(){
...
$(document).ready(function(){
  $('[data-slider]').each(function(){
    new SliderInput($(this));
  });
  $('[data-mirror]').each(function(){
    new Mirror($(this));
  });
  $('[data-book-to-calendar]').each(function(){
    $(this).click(function(){
      loadAndFillCal($('.filter-arrival'));
    })
  });
  $('[data-book-to-flash]').each(function(){
    $(this).click(function(){
      $('.main-price.unavailable').fadeOut().fadeIn();
    });
  })
})
;
...
any_time_manager.registerListWithClasses(
  { 'rental-terms' : 'LoadTermsModal',
    'push-state' : 'AjaxPushState',
    'sortable' : 'AjaxSorter',
    'country-selector' : 'StatePopulator',
    'expand-state' : 'Expandable',
    'end_date' : 'StartDateSetter',
    'form-validate' : 'ValidateForm',
    'prevent-double-click' : 'PreventDoubleLinkClick'
  });
any_time_manager.registerList(
  [ 'range_submitter','hover_overflow','hidey_button','submit-proxy','click_proxy',
    'ajax-browser-push-flash','field-filler','upload_trigger','revealer','upload_csv_trigger',
    'checkbox-hidden-proxy','prevent-double-submit','prevent-double-link-click',
    'date-picker' ] );
...
var LocationDynamicRate = Class.extend({
  init: function(jq_obj){
    if(jq_obj.prop('checked')){
      jq_obj.siblings('form').submit();
    } else {
      jq_obj.siblings('a').click();
    }

  }
})
var CalendarLoader = AjaxLinkSubmission.extend({
  init: function($link){
    this._super($link);
  }
})

var LinkExpander = AjaxLinkSubmission.extend({
  ajaxSuccess: function(data,textStatus,jqXHR){
    this._super(data,textStatus,jqXHR);
    $(this.jq_obj.data('expander')).data('expandable').expand();
  }
})

$(document).on('change','[data-location_dynamic_rate]',function(){
  new LocationDynamicRate($(this));
})

;

var NewLocationFeeSubmission = AjaxFormSubmission.extend({
  ajaxBefore: function(){
    this.vendor_fee_selector = this.jq_obj.find('[data-vendor_fee_selector]')
    this.vendor_fee_value = this.vendor_fee_selector.val();
    this.vendor_fee_option = this.vendor_fee_selector.
      find('option[value=' + this.vendor_fee_value + ']')

  },
  ajaxSuccess: function(data,textStatus,jqXHR){
    this._super(data,textStatus,jqXHR);
    this.vendor_fee_option.remove();

    // selector empty?
    if (this.vendor_fee_selector.find('option').length == 0){
      this.vendor_fee_selector.remove();
       var vendor_id = this.vendor_fee_selector.data('vendor_fee_selector')

       var option = $('#vendor-selector').
         find('option[value=' + vendor_id + ']');

        option.remove();

    }
    // if yes, then remove selector
    // and remove the assoc vendor option
  }
});

//This is a checkbox when clicked adds an LocationBookingChannel association (POST create action)
//when unchecked it deletes the association (DELETE destroy action).
//The override of getAjaxType and getTarget with a check for checked handles those two states.
var LocationBookingChannelSubmission = AjaxFormSubmission.extend({
  getAjaxType: function(){
    if( this.jq_obj.find('input[type=checkbox]').is(':checked') ) {
      return 'post';
    } else { //They just unchecked it, delete association
      return 'delete';
    }
  },
  getTarget: function(){
    if( this.jq_obj.find('input[type=checkbox]').is(':checked') ) {
      this.target = $(this.jq_obj.data('ajax-target'));
    } else { //This is the delete action override
      this.target = $(this.jq_obj.parent());
    }
  }
});

var VendorFeeRevealer = Revealer.extend({
  reveal: function(){
    this._super();
    $('body').append($('#fee_holder').html());
    $('#fee_holder').html(this.$children);
  }
});

var TaxAgencyTaxRevealer = Revealer.extend({
  reveal: function(){
    this._super();
    $('body').append($('#tax_holder').html());
    $('#tax_holder').html(this.$children);
  }
});

var SelectedPhoto = AjaxLinkSubmission.extend({
  ajaxBefore: function(){
    this.jq_obj.siblings().children().removeClass('selected');
    this.jq_obj.children().addClass('selected');
  }
});

var photoRemover = DeleteLink.extend({
  ajaxSuccess: function(data,textStatus,jqXHR){
    this._super(data,textStatus,jqXHR);
    $('#selected_photo_detail').empty();
  }
});

var SearchLocationSetter = Class.extend({
  init: function($setter){
    $('#search_location_name').html($setter.data('name'));
    $('#search_location_id_input').val($setter.data('id'));
    $('#search_location_modal').modal('hide');
  }
});

var ModalSearchLocationSetter = Class.extend({
  init: function($setter){
    $('#property_search_location_id').val($setter.data('id'));
    $('[data-modal_search_location_setter]').removeClass('search_location_highlight');
    $setter.addClass('search_location_highlight');
  }
});

$(document).ready(function(){
  $(document).on('click','[data-thumbnail]',function(e){
    new SelectedPhoto($(this));
  });
  $(document).on('click','[data-search_location_setter]',function(e){
    new SearchLocationSetter($(this));
  });
  $(document).on('click','[data-modal_search_location_setter]', function(e){
    new ModalSearchLocationSetter($(this));
  });

  //Booking Channel Selections on the Units Page
  $(document).on('change','[data-bc-selector]',function(e){
    $(this).closest('form').submit();
  });

  $(document).on('change','#line_item_chargeable_id',function(e){
    $('#line_item_chargeable_type').val($(this.options[this.selectedIndex]).data('chargeable-type'))
  })

  $(document).on('change','#coupon_payer_id',function(e){
    $('#coupon_payer_type').val($(this.options[this.selectedIndex]).data('payer-type'))
  })

});
var ModalDelete = DeleteLink.extend({
  ajaxSuccess: function(data,textStatus,jqXHR){
    this._super(data,textStatus,jqXHR);
    $('button.close').click();
  }
...
var BookingChannelAmenityModalDelete = DeleteLink.extend({
  ajaxSuccess: function(data,textStatus,jqXHR){
    $('button.close').click();
    $(this.target[0]).find('a').text('Add');
  },
  ajaxBefore: function(jqXHR){
    //noop
  }
});
var StatePopulator = Class.extend({
  init: function($country_select) {
    this.$country_select = $country_select;
    $country_select.val("United States");
    this.populateStates();
    var state_populator = this;
    $country_select.bind('change', function() {
      state_populator.populateStates();
    });
  },
  populateStates: function(){
    var country = this.$country_select.val();
    if(country){
      var selected_state = this.$country_select.data('selected-state');
      var addressable_type = this.$country_select.data('addressable-type');
      var address_type = this.$country_select.data('address-type');
      var patt=new RegExp("manage");
      if(patt.test(window.location.pathname)){
        $.get('/manage/addresses/states_for/' + country + '?addressable_type=' + addressable_type + '&address_type=' + address_type + '&selected_state=' + selected_state)
        .success(function(data) {
          $('[data-state-dropdown]').html(data);
        });
      } else {
        $.get('/addresses/states_for/' + country + '?addressable_type=' + addressable_type + '&address_type=' + address_type + '&selected_state=' + selected_state)
        .success(function(data) {
          $('[data-state-dropdown]').html(data);
        });
      }
...
var ValidateForm = Class.extend({
  init: function(obj) {
    el = $(obj).validate();
  }
})

var LoadTermsModal = Class.extend({
  init: function(obj) {
    obj.click(function() {

      el = $("[data-terms-modal]").show();
      if (!el.is(":visible")) {
        el.show();
      }
    });
...
var DatePickerLoader = Class.extend({
  init: function(jq_obj){
    jq_obj.datepicker({
...
var FrontEndDatePickerLoader = Class.extend({
  init: function($calendar_control){
    if($calendar_control.hasClass('filter-arrival')){
      $input = $calendar_control.find('input:first')
      var start_date_loader = new FrontEndStartDatePickerLoader($input);
    }
...
var FrontEndStartDatePickerLoader = Class.extend({
...
var UpdateFilterDate = Class.extend  ({
  init: function(jq_obj){
    var selected_date = jq_obj.find('.date').data('calender_date');
    var filterArrival   = $('.filter-arrival');
    var arrivalInput = filterArrival.find('input');
    $(".filter-arrival input").datepicker("setDate", selected_date);
    filterArrival.find('.filter-selection').text(selected_date);
    loadAndFillCal($('.filter-departure'));
    changeFilterBarPosition();
  }
})

var ToggleFullYearCal = Class.extend ({
  init: function(jq_obj) {
    allCalenders = $("[data-calender_prop]");
    allCalenders.not("[data-calender_main='true']").fadeToggle("slow", "linear");
  }
});

var ToggleFullAmenities = Class.extend ({
  init: function(jq_obj) {
    $("[ data-all_amenities]").fadeToggle("slow", "linear");
  }
});
var ToggleFullReviews = Class.extend ({
  init: function(jq_obj) {
    $("[data-review_main='true']").fadeToggle("slow", "linear");
  }
});

var ToggleFoldText = Class.extend ({
  init: function(jq_obj) {
    if (jq_obj.attr('src') == "/images/more_fold.png") {
      jq_obj.attr('src', "/images/less_fold.png");
    } else {
      jq_obj.attr('src', "/images/more_fold.png");
    }
  }
});

var ToggleMapText = Class.extend ({
  init: function(jq_obj) {
    console.log('toggling...')
    if (jq_obj.attr('src') == "/images/map_fold.png") {
      jq_obj.attr('src', "/images/photo_fold.png");
    } else {
      jq_obj.attr('src', "/images/map_fold.png");
    }
  }
});

var ToggleMap = Class.extend ({
  init: function(jq_obj) {
    if($('[data-map_displayed]').data('map_displayed') == false) {
      $('.sprop-map').css('position', 'relative').css("right", 0);
      $('[data-sprop_slider]').css('position', 'absolute').css("right", -9000);
      $('[data-map_displayed]').data('map_displayed', true)
    } else {
      $('.sprop-map').css('position', 'absolute').css("right", -9000);
      $('[data-sprop_slider]').css('position', 'relative').css("right", 0);
      $('[data-map_displayed]').data('map_displayed', false)
    }
  }
});
...
$(document).ready(function(){
  $(document).on('click', '[data-map_displayed]', function(e){
    new ToggleMap($(this));
  });

  $(document).on('click','[data-calender_day]',function(e){
    new UpdateFilterDate($(this));
  });
  $(document).on('click', '[data-calenders_displayed]', function(e){
    new ToggleFullYearCal($(this))
  });
  $(document).on('click', '[data-amenities_displayed]', function(e){
    new ToggleFullAmenities($(this))
  });
  $(document).on('click', '[data-reviews_displayed]', function(e){
    new ToggleFullReviews($(this))
  });

  $(document).on('click', '[data-map_fold]', function(e){
    new ToggleMapText($(this));
  });

  $(document).on('click', '.more-fold', function(e){
    new ToggleFoldText($(this));
  });

  $('.search-pattern-bg').show(0,
    function(){$('.search-background').fadeIn('slow');}
  );
});
$(document).ready(function() {
  if (typeof(Zenbox) !== "undefined") {
    Zenbox.init({
      dropboxID:   "20199313",
      url:         "https://vrpartner.zendesk.com",
...
function initialize() {
  ...
  var mapOptions = {
    zoom: 14,
    center: new google.maps.LatLng(mapDiv.data('property_latitude'), mapDiv.data('property_longitude')),
  ...
  var latLong = new google.maps.LatLng(mapDiv.data('property_latitude'), mapDiv.data('property_longitude'))
  ...
  markerBounds.extend(new google.maps.LatLng(mapDiv.data('property_latitude'), mapDiv.data('property_longitude')));

  $(".sprop-mp").each(function(index) {
    var latLong = new google.maps.LatLng($(this).data("mp_latitude"), $(this).data('mp_longitude'))
    var map_point = new google.maps.Marker({
      position: latLong,
      map: map,
...
if ($("[data-sprop_map]").length != 0) {
  google.maps.event.addDomListener(window, 'load', initialize);
  $('[data-sprop_map]').css("position", "absolute").css("right", -9000);
}

;
function prep_inquiry_tester() {
  if ($('#inquiry_test #provider').length != 0) {
    var setup_inquiry_form = function(elem) {
      var data = inquiry_data[elem.val()]
      $('#inquiry_test #inquiry_from').val(data['from'])
      $('#inquiry_test #inquiry_text').val(data['text'])
      $('#inquiry_test #inquiry_headers').val(data['headers'])
    }

    $('#inquiry_test #provider').on('change', function() {
      setup_inquiry_form($(this))
    })

    setup_inquiry_form($('#inquiry_test #provider'))
  }
}
;
function resendNotification(resend_url, button_selector){
  $.ajax({
    url: resend_url,
    type: "GET",
    data: {},
    beforeSend: function(jqXHr) { $(button_selector).attr("disabled", "disabled"); },
    success: function(data,textStatus,jqXHR) {
      setTimeout(function(){ $(button_selector).removeAttr("disabled"); }, 3000)
    },
    error: function(jqXHr) { $(button_selector).removeAttr("disabled"); }
  });
}
;
...
$(document).ready(function(){
  $('#new_payment').submit(function(e){
    ...
    check_for_form('#user-form form', 'Please complete Step 1')
    check_for_form('#group-info-form form', 'Please complete Step 2')
    check_for_form('#terms-form form', 'Please complete Step 3')
    check_for_form('#address-form form', 'Please complete Step 4')
    check_for_form('#ccp-form form', 'Please complete Step 5')

    if ($('#errors').text().length > 0) {
      e.preventDefault();
    } else {
      if ($('#new_payment').find('[data-loading-visible="false"]').length > 0) {
        $('#new_payment').find('[data-loading-visible="false"]').addClass('hidden');
      }
      if ($('#new_payment').find('[data-loading-visible="true"]').length > 0) {
        $('#new_payment').find('[data-loading-visible="true"]').removeClass('hidden');
      }
    }
  });
});

var form_map = ['#user-form',
                '#user-form',
                '#group-info-form',
                '#terms-form',
                '#address-form',
...
function check_for_form(form, message) {
  if ($(form).length > 0) {
...
$(document).ajaxComplete(function(){
  $('[data-step]').each(function(){
    if(!$(this).data('clicked')) {
      ...
      $('[data-step-link="' + $(this).data('step') + '"]').click();
...
$(document).ready(function() {
  if ($('[data-suppress-js-console]').length > 0) {
    console.log = function(){};
  }
});
// This is a manifest file that'll be compiled into application.js, which will include all the files
